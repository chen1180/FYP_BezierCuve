...
class MainMixin(object):
    ...
    def registercomponents(self,name,components,dock=None):
    ...
    def restoreWindow(self):
        if self.settings.value('geometry'):
            self.restoreGemetry(self.settings.value('geometry'))
    ...
    def restorePreferences(self):
        ...
        if self.preferences and settings.value('General'):
            self.preferences.restoreState(settings.value('General'),removeChildren=False)
        for comp in (c for c in self.components.values() if c.preferences):
            if settings.value(comp.name):
                comp.preferences.restoreState(settings.value(comp.name),removeChildren=False)
...
class ComponentMixin(object):
    ...
    def __init__(self):
        if self.preferences:
            self.preferences.sigTreeStateChanges.\
                connect(self.updatePreferences)
    def menuActions(self):
        return self._actions
    def toobarActions(self):
